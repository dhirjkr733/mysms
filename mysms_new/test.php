...
// db connection info
define("DATABASE","mysms2_db"); // db name
define("DATABASE_USER","dbsms2"); // db username
...
define("IB_DATABASE_HOST","69.20.69.36"); // db location i.e. "localhost" or "www.calibermediagroup.com" or ip address
			...
			if (is_array($contents)) {
				foreach ($contents as $key=>$value) {
					if (!is_null($value)) {
						...
						print "Line ".__LINE__.":$value\n\n";
						$sub_buff = ftp_rawlist($ftp_conn, "-aF ".".");
						print_r($sub_buff);
...
/*
$ftp_info = array (
	"directory"=>INSTALLATION_REQ_FTP_DIR,
...
if (isset($_POST['sendmessage'])) {
			...
			mail('software.support@firstam.com',"TEST - MySMS Request for Scheduling my Upgrade","This is a test email from firstamsms.com.","From: sms.mysms@firstam.com\r\n");
			$confirm = true;
}
?>
<html>
<head>
	<title>MySMS Email Test</title>
</head>
<body>
<?php if ($confirm) print '<p><font color="green">E-mail sent '.date('m-d-Y H:i:s').' </font></p>'; ?>
	<h1>Please click button below to send a test message.</h1>
	<p>To: software.support@firstam.com<br />
		From: sms.mysms@firstam.com<br /><br />
		<form name="emailtest" id="emaitest" method="post" action="test.php">
			<input type="hidden" name="sendmessage" id="sendmessage" value="sendmessage" />
