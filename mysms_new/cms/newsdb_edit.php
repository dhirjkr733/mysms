...
$form_fields_ary = array (
	...
	"heading" 		=> array ("label"=>"Content",				"error"=>"",		"value"=>"$heading",		"size"=>"3", "maxlength"=>"100", "type"=>"text_area", "required"=>"Required", "msg"=>"",	"validate"=>"check_full,strip_spaces,strip_bad_char"),
...
$select_existing_query = "SELECT id,heading FROM news_items ORDER BY heading ASC "; // NOTE: showing ALL active records (no status flag)
...
if ($update || $add) {
	...
	if ($add) {
		...
		$protect_sql = "SELECT id FROM news_items WHERE heading='$heading'";
	...
	if (!$error) {
		...
		if ($add) {
			...
			$sql2 = "INSERT INTO news_items (heading,link) VALUES ('$heading','$link')";
		...
		if ($update) {
			...
			$sql = "UPDATE news_items SET heading='$heading',link='$link' WHERE id=$id";
...
if ($delete) {
	...
	$delete_query3 = "DELETE FROM news_items WHERE id=$id"; 
