...
$this_page_title = "Contact Implementation Coordinator Region Database";
...
$form_fields_ary = array (
	...
	"heading" 		=> array ("label"=>"Heading",			"error"=>"",		"value"=>"$heading",		"size"=>"30", "maxlength"=>"75", "type"=>"text",				"required"=>"Required", "msg"=>"",	"validate"=>"check_full,strip_spaces,strip_bad_char"),

	"state_id" 		=> array ("label"=>"State",			"error"=>"",		"value"=>"$state_id",			"size"=>"30", "maxlength"=>"50", "type"=>"state_pulldown",				"required"=>"", "msg"=>"")
...
$select_existing_default = "Select Region...";
...
$select_existing_query = "SELECT regions.id,CONCAT(regions.state_id, ' - ', regions.heading) as heading FROM regions ORDER BY heading"; // NOTE: showing ALL active records (no status flag)
...
if ($update || $add) {
	...
	if ($add) {
		...
		$protect_sql = "SELECT id FROM regions WHERE heading='$heading'";
	...
	if (!$error) {
		...
		if ($add) {
			...
			$sql2 = "INSERT INTO regions (id,heading,state_id) VALUES (NULL,'$heading','$state_id')";
		...
		if ($update) {
			...
			$sql = "UPDATE regions SET heading='$heading',state_id='$state_id' WHERE id=$id";
...
if ($change) {
	...
	$sql = "SELECT id,heading,state_id FROM regions WHERE id=$id";
...
if ($delete) {
	...
	$delete_query3 = "DELETE FROM regions WHERE id=$id"; 
