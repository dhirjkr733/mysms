...
$form_fields_ary = array (
	...
	"SMS_rate" 		=> array ("label"=>"SMS Rate",			"error"=>"",		"value"=>"$SMS_rate",		"size"=>"20", "maxlength"=>"20", "type"=>"text", ""),

	"notes" 		=> array ("label"=>"Notes",				"error"=>"",		"value"=>"$notes",			"size"=>"3", "maxlength"=>"100", "type"=>"text_area",""),

	"facility_id" 	=> array ("label"=>"Facility",			"error"=>"",		"value"=>"$facility_id",		"size"=>"30", "maxlength"=>"50", "type"=>"facility_pulldown", "required"=>"Required", "msg"=>"",	"validate"=>"check_full,strip_spaces")
...
$select_existing_default = "Select Hotel...";
...
$select_existing_query = "SELECT hotels.id, CONCAT(facilities_directory.heading,' - ', hotels.heading) AS description FROM hotels, facilities_directory WHERE hotels.facility_id = ".$_REQUEST['id']." AND facilities_directory.id = ".$_REQUEST['id']." ORDER BY description ASC "; // NOTE: showing ALL active records (no status flag)
...
if ($update || $add) {
	...
	if ($add) {
		...
		$protect_sql = "SELECT id FROM hotels WHERE heading='$heading'";
	...
	if (!$error) {
		...
		if ($add) {
			...
			$sql2 = "INSERT INTO hotels (id,heading,address1,address2,phone,ext,fax,SMS_rate,notes,facility_id) VALUES (NULL,'$heading','$address1','$address2','$phone','$ext','$fax','$SMS_rate','$notes','$facility_id')";
		...
		if ($update) {
			...
			$sql = "UPDATE hotels SET heading='$heading',address1='$address1',address2='$address2',phone='$phone',ext='$ext',fax='$fax',SMS_rate='$SMS_rate',notes='$notes',facility_id='$facility_id' WHERE id=$id";
...
if ($delete) {
	...
	$delete_query3 = "DELETE FROM hotels WHERE id=$id";
